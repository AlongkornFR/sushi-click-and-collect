...
export function CartProvider({ children }) {
  ...
  const addItem = (product, qty = 1) => {
    ...
    setItems((prev) => {
      ...
      if (existing) {
        return prev.map((x) =>
          x.id === product.id ? { ...x, quantity: x.quantity + quantityToAdd } : x
        )
      }
      return [
        ...prev,
  ...
  const setQuantity = (productId, quantity) => {
    ...
    setItems((prev) => prev.map((x) => (x.id === productId ? { ...x, quantity: q } : x)))
  }

  const increment = (productId) => {
    setItems((prev) =>
      prev.map((x) => (x.id === productId ? { ...x, quantity: x.quantity + 1 } : x))
    )
  }
  ...
  const decrement = (productId) => {
    setItems((prev) =>
      prev.map((x) =>
        x.id === productId ? { ...x, quantity: Math.max(1, x.quantity - 1) } : x
      )
    )
  }
